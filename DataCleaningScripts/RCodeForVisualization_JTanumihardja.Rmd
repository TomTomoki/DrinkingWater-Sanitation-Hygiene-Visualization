...
summary(ets_model)
```
ETS(M,A,N) means the exponential smoothing model (ETS) has:
...
RMSE is pretty low so the trend is quite good. 
...
autoplot(ets_forecast, predict.size = 1, 
  ...
  autolayer(fitted(ets_model))+
  theme_bw()+
  theme(text=element_text(size=15))+
  expand_limits(y=100)+
  labs(title = "Time Series Forecast - Exponential Smoothing (ETS)",
       subtitle = paste("2030 World 'At least basic' ES forecast =",
...
autoplot(holt_forecast, predict.size = 1, 
         predict.colour = 'blue', predict.linetype = 'dashed',
         conf.int = TRUE, conf.int.fill = "lightblue") + 
  autolayer(fitted(holt_forecast), series='Fitted') +
  theme_bw()+
